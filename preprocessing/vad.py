...
class VoiceActivationDetector:
    ...
    def determine_frame_ranges(self, data):
    ...
    def merge_frame_ranges(self, ranges, fs):
        merged = list()
    ...
    def perform_vad(self, wav_file, debug = False):
        ...
        pool = ThreadPool(2)
        results_wrapped = pool.map(self.moving_average, \
        ...
        pool = ThreadPool(4)
        results_wrapped = pool.map(self.determine_frame_ranges, chunks)
        results_unwrapped = []
